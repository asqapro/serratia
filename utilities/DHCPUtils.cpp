...
std::vector<pcpp::IPv4Address> serratia::utils::parseIPv4Addresses(const pcpp::DhcpOption* option) {
    ...
    const size_t data_len = option->getDataSize(); //length in bytes
...
bool serratia::utils::RealPacketSender::send(const pcpp::Packet& packet) {
...
void serratia::utils::DHCPServer::stop() const {
...
void serratia::utils::DHCPServer::handleDiscover(const pcpp::Packet& dhcp_packet) {
    ...
    const auto src_mac = listener_->getMacAddress();
    ...
    const auto src_ip = listener_->getIPv4Address();
    ...
    if (const auto client_id_option = dhcp_layer->getOptionData(pcpp::DHCPOPT_DHCP_CLIENT_IDENTIFIER); client_id_option.isNotNull()) {
    ...
    const serratia::protocols::DHCPOfferConfig dhcp_offer_config(dhcp_common_config, dhcp_header->hops,
                                                            dhcp_header->transactionID, lease.assigned_ip,
