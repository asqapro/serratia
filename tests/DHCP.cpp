...
TEST_CASE( "Build DHCP packets" ) {
    ...
    SECTION( "DHCP offer" ) {
        ...
        serratia::protocols::DHCPOfferConfig dhcp_offer_config(dhcp_common_config, env.hops, env.transaction_id,
                                                               your_ip, server_id, env.seconds_elapsed,
                                                               env.bootp_flags, env.server_ip, env.gateway_ip,
                                                               server_name, boot_name,
                                                               env.lease_time.count(), env.subnet_mask,
                                                               env.routers, env.dns_servers,
    ...
    SECTION( "DHCP initial request" ) {
        ...
        serratia::protocols::DHCPRequestConfig dhcp_request_config(dhcp_common_config, env.transaction_id, env.hops,
                                                                   env.seconds_elapsed, env.bootp_flags,
                                                                   env.gateway_ip, client_id, param_request_list,
                                                                   env.client_host_name, std::nullopt,
    ...
    SECTION( "DHCP ACK" ) {
        ...
        std::copy_n(env.server_host_name.begin(), std::min(env.server_host_name.size(), server_name.size()), server_name.begin());
        //TODO: Use env stuff instead of defining here
        std::array<std::uint8_t, 128> boot_file_name = {0};

        serratia::protocols::DHCPAckConfig dhcp_ack_config(dhcp_common_config, env.transaction_id, env.client_ip,
                                                           env.server_ip, env.lease_time.count(),
                                                           env.hops, env.seconds_elapsed, env.bootp_flags,
                                                           env.server_ip, env.gateway_ip,
                                                           server_name, boot_file_name,
                                                           env.subnet_mask, env.routers, env.dns_servers,
                                                           env.renewal_time.count(),
                                                           env.rebind_time.count());
        ...
        REQUIRE( dhcp_layer->getOptionData(pcpp::DHCPOPT_DHCP_LEASE_TIME).getValueAs<std::uint32_t>() == ntohl(env.lease_time.count()) );
        REQUIRE( dhcp_layer->getOptionData(pcpp::DHCPOPT_SUBNET_MASK).getValueAsIpAddr() == env.subnet_mask );
        REQUIRE( dhcp_layer->getOptionData(pcpp::DHCPOPT_ROUTERS).getValueAsIpAddr() == env.server_ip ); //TODO: double check this
